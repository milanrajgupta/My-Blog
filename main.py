...
def dashboard():
    ...
    if request.method == 'POST':    # Redirect to Admin Panel
        ...
        if(username == params['admin_user'] and userpass == params['admin_password']):    #set the session variable
...
def contact():
    if(request.method == 'POST'):
        ...
        db.session.commit()
        mail.send_message('New message from my blog  ',
                          sender = email, recipients = [params['gmail-user']],
                          body = message + "\n" + phone
                          )
        flash("Thank you!", "success")
   # else:
        #flash("Your Details are not be Submitted!", "danger")
